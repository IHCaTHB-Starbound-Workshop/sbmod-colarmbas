...
function build(directory, config, parameters, level, seed)
  ...
  if builderConfig.palette1 then
    local palette1 = root.assetJson(util.absolutePath(directory, builderConfig.palette1))
    local selectedSwaps = randomFromList(palette1.swaps, seed, "paletteSwaps")
    for k, v in pairs(selectedSwaps) do
      config.paletteSwaps = string.format("%s?replace=%s=%s", config.paletteSwaps, k, v)
    end
  end
  if builderConfig.palette2 then
    local palette2 = root.assetJson(util.absolutePath(directory, builderConfig.palette2))
    local selectedSwaps = randomFromList(palette2.swaps, seed, "paletteSwaps")
    for k, v in pairs(selectedSwaps) do
      config.paletteSwaps = string.format("%s?replace=%s=%s", config.paletteSwaps, k, v)
    end
  end
  if builderConfig.palette3 then
    local palette3 = root.assetJson(util.absolutePath(directory, builderConfig.palette3))
    local selectedSwaps = randomFromList(palette3.swaps, seed, "paletteSwaps")
    for k, v in pairs(selectedSwaps) do
      config.paletteSwaps = string.format("%s?replace=%s=%s", config.paletteSwaps, k, v)
    end
  end
  if builderConfig.palette4 then
    local palette4 = root.assetJson(util.absolutePath(directory, builderConfig.palette4))
    local selectedSwaps = randomFromList(palette4.swaps, seed, "paletteSwaps")
    for k, v in pairs(selectedSwaps) do
      config.paletteSwaps = string.format("%s?replace=%s=%s", config.paletteSwaps, k, v)
    end
  end
